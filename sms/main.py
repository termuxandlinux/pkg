...
path = os.getcwd()
path_token = path+"$HOME/.sms/_token.txt"
...
os.system('Title  Sms-sender ║ BY Alex')
...
def setup_token():
  ...
  if os.path.getsize(path_token) == 0:
    ...
    fichier = open("$HOME/.sms/_token.txt", "a")
...
def delete_token():
  ...
  if os.path.getsize(path_token) == 0:
    ...
    token = open("$HOME/.sms/_token.txt", "r")
    token = token.read()
    ...
    if sur ==2:
      os.system('cls' if os.name == 'nt' else 'clear')
      sms_sender()
      os.remove(f'$HOME/.sms/_token.txt')
      fichier = open("$HOME/.sms/_token.txt", "a")
      fichier.close()
...
def yestoken():
  ...
  fichier = open("$HOME/.sms/log.txt", "a")
  fichier.write(f"\n{date}\nPHONE:{phone}\nMESSAGE:{texte}\n")
  fichier.close()
...
def log():
  ...
  fichier = open("$HOME/.sms/log.txt", "r")
  print(fichier.read())
  fichier.close()
