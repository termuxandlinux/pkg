...
void add_command(const char *botname, const char *input, const char *output);
void add_bot_token(const char *botname, const char *token);
...
int main(int argc, char *argv[]) {
    if (argc > 1) {
        if (strcmp(argv[1], "--setup") == 0) {
            ...
            strncpy(botname, argv[1], MAX_NAME_LENGTH);
            botname[MAX_NAME_LENGTH - 1] = '\0';
            char prefix[10];
            ...
            do {
                ...
                scanf("%s", output);
                add_command(botname, command, output);
